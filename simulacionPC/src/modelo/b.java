...
public class b {
    ...
    private int altura;
    private int niveles;
    private LinkedList<Node> listAnchura = new LinkedList<>();
    private LinkedList<Node> listProfundidad = new LinkedList<>();
    private static final int T = 2;
    private Modelo.Node mRootNode;
    private static final int LEFT_CHILD_NODE = 0;
    private static final int RIGHT_CHILD_NODE = 1;

    public b() {
        mRootNode = new Modelo.Node(T);
        mRootNode.mIsLeafNode = true;
    }

    public void add(int key, Object object) {
        Modelo.Node rootNode = mRootNode;
        if (!update(mRootNode, key, object)) {
            if (rootNode.mNumKeys == (2 * T - 1)) {
                Modelo.Node newRootNode = new Modelo.Node(T);
    ...
    void splitChildNode(Modelo.Node parentNode, int i, Modelo.Node node) {
        Modelo.Node newNode = new Modelo.Node(T);
    ...
    void insertIntoNonFullNode(Modelo.Node node, int key, Object object) {
    ...
    public void delete(Modelo.Node node, int key) {
        if (node.mIsLeafNode) { // 1. If the key is in node and node is a leaf node, then delete the key from node.
            ...
            if ((i = node.binarySearch(key)) != -1) { // 2. If node is an internal node and it contains the key... (key is i-th key of node if node contains key)                   
                Modelo.Node leftChildNode = node.mChildNodes[i];
                Modelo.Node rightChildNode = node.mChildNodes[i + 1];
                if (leftChildNode.mNumKeys >= T) { // 2a. If the predecessor child node has at least T keys...
                    Modelo.Node predecessorNode = leftChildNode;
                    Modelo.Node erasureNode = predecessorNode; // Make sure not to delete a key from a node with only T - 1 elements.
                    ...
                    Modelo.Node successorNode = rightChildNode;
                    Modelo.Node erasureNode = successorNode; // Make sure not to delete a key from a node with only T - 1 elements.
                ...
                Modelo.Node childNode = node.mChildNodes[i]; // childNode is i-th child of node.                               
                if (childNode.mNumKeys == T - 1) {
                    Modelo.Node leftChildSibling = (i - 1 >= 0) ? node.mChildNodes[i - 1] : null;
                    Modelo.Node rightChildSibling = (i + 1 <= node.mNumKeys) ? node.mChildNodes[i + 1] : null;
    ...
    int mergeNodes(Modelo.Node dstNode, Modelo.Node srcNode) {
        int medianKeyIndex;
    ...
    void moveKey(Modelo.Node srcNode, int srcKeyIndex, int childIndex, Modelo.Node dstNode, int medianKeyIndex) {
    ...
    public Object search(Modelo.Node node, int key) {
        int i = 0;
    ...
    public Object search2(Modelo.Node node, int key) {
        while (node != null) {
            int i = 0;
            while (i < node.mNumKeys && key > node.mKeys[i]) {
                i++;
    ...
    private boolean update(Modelo.Node node, int key, Object object) {
        while (node != null) {
            int i = 0;
            while (i < node.mNumKeys && key > node.mKeys[i]) {
                i++;
    ...
    String printBTree(Modelo.Node node) {
        String string = "";
        if (node != null) {
            if (node.mIsLeafNode) {
    ...
    // Inorder walk over the tree.
    ArrayList<Integer> getKeys(Modelo.Node node) {
    ...
    private void recorrido() {
        ...
        LinkedList<Node> auxPro = new LinkedList<>();

        setListAnchura(recorridoAnchura(cola, mRootNode));
        System.out.println("anchura"+getListAnchura().size());
        setListProfundidad(recorridoProfundidad(mRootNode, auxPro, 1));
        System.out.println("anchura"+getListProfundidad().size());
    ...
    private LinkedList<Node> recorridoAnchura(LinkedList<Node> cola, Node nodo) {
        ...
        while (!cola.isEmpty()) {
            ...
            aux = cola.remove();
            if (aux.mChildNodes.length > 0) {
                int conteoHijos = 0;
                for (int i = 0; i < aux.mChildNodes.length; i++) {

                    int conteoObjetos = 0;
                    if (aux.mChildNodes[i] != null) {
                        for (int j = 0; j < aux.mChildNodes[i].mObjects.length; j++) {
                            if (aux.mChildNodes[i].mObjects[j] != null) {
                                conteoObjetos++;
                            }
                        }
                        //
                        System.out.println("nodo " + i + " con " + conteoObjetos + " objetos");

                        conteoHijos++;
                        cola.add(aux.mChildNodes[i]);
                        recorrido.add(aux.mChildNodes[i]);
                    }
                   // System.out.println("nodo " + i + " con " + conteoHijos + " hijos");
                }

            }

        }
        recorrido.addFirst(getmRootNode());
        return recorrido;
    }
    ...
    private LinkedList<Node> recorridoProfundidad(Node nodo, LinkedList<Node> listAux, int auxAltura) {
        ...
        if (nodo.isLleaf()) {
            ...
            for (int i = 0; i < nodo.mChildNodes.length; i++) {
                if (nodo.mChildNodes[i]!=null) {
                listAux = recorridoProfundidad(nodo.mChildNodes[i], listAux, auxAltura + 1);
